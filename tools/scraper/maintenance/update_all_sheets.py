...
# -*- coding: utf-8 -*-
"""
全シート地区・GoogleマップURL更新ツール

parkingsとtoiletsシートも含めて、全シートの地区とGoogleマップURLを更新します。
"""
...
def update_sheet(spreadsheet, sheet_name):
    """指定シートの地区・GoogleマップURLを更新"""
    ...
    try:
        ...
        print(f"   地区: {district_index} → {column_number_to_letter(district_index) if district_index is not None else 'N/A'}")
        print(f"   GoogleマップURL: {maps_url_index} → {column_number_to_letter(maps_url_index) if maps_url_index is not None else 'N/A'}")
        ...
        print(f"❌ {sheet_name}シート更新エラー: {e}")
        return False

def update_all_sheets():
    """全シートの地区・GoogleマップURL自動設定"""
    
    print("🔧 全シート地区・GoogleマップURL更新ツール")
    print("=" * 70)
    ...
    try:
        ...
        for sheet_name in target_sheets:
            if update_sheet(spreadsheet, sheet_name):
                success_count += 1
        
        print(f"\n🎉 全シート更新完了！")
        print(f"   ✅ 成功: {success_count}/{len(target_sheets)}シート")
