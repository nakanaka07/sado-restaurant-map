# ADR-003: Scraper アーキテクチャ再設計

## ステータス

**Accepted** - 2025 年 8 月 27 日

## コンテキスト

佐渡飲食店マップの`tools\scraper`モジュールは、Google Places API からデータを収集し、Google Sheets に更新する Python システムです。現在のシステムには以下の課題があります：
...
**この決定により、`tools\scraper`は現代的で保守性の高いアーキテクチャに生まれ変わり、プロジェクト全体の品質向上に貢献します。**
