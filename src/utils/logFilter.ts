...
export function setupLogFiltering(): void {
  ...
  const shouldFilterMessage = (message: string): boolean => {
    // Workboxログの検出
    if (message.includes('workbox') || message.includes('sw.js') ||
        message.includes('Service Worker') || message.includes('vt/pb=')) {
  ...
  console.warn = (...args: any[]) => {
    const message = args.join(' ');
    if (!shouldFilterMessage(message) || config.workbox === 'warn' || config.googleMaps === 'warn') {
  ...
  // 開発者向けメッセージ
  console.info('🔧 [LogFilter] Workbox logs filtered for development environment');
}
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.addEventListener('message', (event) => {
      ...
      if (event.data && typeof event.data === 'object' &&
          (event.data.type || '').includes('workbox')) {
