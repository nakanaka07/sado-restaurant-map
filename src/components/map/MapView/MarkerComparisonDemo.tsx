 ...
 */
export function MarkerComparisonDemo({ points, onMarkerClick }: MarkerComparisonDemoProps) {
  ...
  const renderMarker = (point: MapPoint, index: number) => {
    ...
    switch (selectedMarkerType) {
      ...
      case 'enhanced-png':
        return <EnhancedPNGMarker key={key} point={point} onClick={onMarkerClick} />;
      case 'svg':
        return <SVGMarkerSystem key={key} point={point} onClick={onMarkerClick} />;
      default:
        return <EnhancedPNGMarker key={key} point={point} onClick={onMarkerClick} />;
    }
  };
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      >
        <h4 style={{ margin: '0 0 8px', fontSize: '14px' }}>表示中のマーカー</h4>
        <p style={{ margin: '0' }}>
          レストラン: {samplePoints.filter(p => p.type === 'restaurant').length}件<br />
          駐車場: {samplePoints.filter(p => p.type === 'parking').length}件<br />
